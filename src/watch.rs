...
pub async fn watch_table_changes(pool: &PgPool, options: Options, status: Arc<AtomicBool>, mut rx: Receiver<ChangeEvent>, tx: Sender<String>) -> anyhow::Result<()> {
  ...
  W: io::Write + Send,
{
  ...
  tokio::spawn(
    async move { let pool = connect_db(&db_url.clone()).await;
  ...
  for o in t_options.iter_mut() {
    let a: &str = o.name;
    match a {
      "m" => {o.status = model; if model {let _ = task_tx_clone.send(ChangeEvent::M).await;}}
      "c" => {o.status = create; if create {let _ = task_tx_clone.send(ChangeEvent::C).await;}}
      "r" => {o.status = retrieve; if retrieve {let _ = task_tx_clone.send(ChangeEvent::R).await;}}
      "u" => {o.status = update; if update {let _ = task_tx_clone.send(ChangeEvent::U).await;}}
      "d" => {o.status = delete; if delete {let _ = task_tx_clone.send(ChangeEvent::D).await;}}
